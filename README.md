...
Captain Cook accepts a protein sequence or a short natural-language request and returns a single result unit: one image and one clinically readable caption. Folding uses Biohub ESMFold2. Structure prediction, binding scores, and small-molecule (**ligand**) design use Boltz (`boltz-2.1` and the design API). Protein **binder** design uses BindCraft via optional Modal compute-only jobs (fail-closed until configured). All scores are computational estimates. A pre-compute biosecurity gate (`commec`) classifies DNA and RNA before paid GPU work; missing binaries or databases fail closed with distinct operator-facing refuse strings. `/research` returns a preprint literature brief (bioRxiv and medRxiv). `/evidence` returns a peer-reviewed MEDLINE brief with preprints excluded. Both use Europe PMC and Harvard references. `/scribe` organises user-supplied meeting text into structured minutes; unlinked minutes sit in a user inbox until assigned to a patient. `/measure` records paste-friendly observations on the card with curated clinical abbreviations. Per patient, a background sorter maintains exactly two core files — `clinic.md` (clinical prose, including peer-reviewed Evidence and Chromosomal notes) and `lab.ipynb` (structures, designs, preprint literature, parts annotations) — plus a `search.json` sidecar for fast DOI, section, and feature-key lookup (never full sequence bodies). Session biometrics and sequences stay secret on the card and never appear as raw values in those files, language-model prompts, research briefs, Discord, or `/app` HTML. `/app` (with `/board` alias) builds a case-conference packet and optional short-TTL live view with Clinical Chromosomal and Laboratory Parts SEQ|TABLE panels. `/annotate` accepts CNV coordinates, raw sequence, or NGS file upload for Chromosomal copy-number briefs (ClassifyCNV); `/annotate parts` maps genetic parts via Bakta when databases are installed, otherwise fail-closes. `/variant` and `/trials` support variant literature and public trial shortlists. The agent does not diagnose disease, recommend therapy, or plan synthesis or wet-lab work. Operators remain responsible for Biohub and Boltz acceptable-use policies and for applicable law.
...
| Annotate | `/annotate` three intakes on a loaded card: (1) one-shot CNV coords / BED / VCF-SV → ClassifyCNV Chromosomal brief; (2) raw DNA/RNA paste → secure SEQUENCE attribute (length+hash in chat); (3) FASTA/FASTQ-capped/VCF/BED upload — not BAM/CRAM. `/annotate parts` → Bakta (+ Pyrodigal) GenBank/GFF genetic-parts map after `commec`; promoter/RBS/terminator via curated match when present. Not genetic-parts on bare `/annotate`; not a diagnosis. Fail-closed when `COMMEC_BIN`, Bakta, or their databases are missing. Shared annotate DBs (when installed) live on private R2, cached on the poller — never GitHub, never `/app` public. |
...
Shared Bakta/`commec` databases (when installed) belong on **private Cloudflare R2**, cached on the poller host — not in GitHub, not on public `/app` URLs.
